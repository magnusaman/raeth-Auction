...
export default function TeamHealthBar({ current, max, label, className = "" }: TeamHealthBarProps) {
  ...
  const colors = {
    high: { bar: "linear-gradient(90deg, #D4A853, #E8D5A3)", glow: "rgba(212,168,83,0.2)" },
    mid: { bar: "linear-gradient(90deg, #F59E0B, #D4A853)", glow: "rgba(245,158,11,0.2)" },
  ...
  return (
    <div className={`flex items-center gap-2 ${className}`}>
      ...
      {label && (
        <span className="text-[11px] font-mono text-[#A09888] tabular-nums shrink-0">
          {label}
        </span>
      )}
    </div>
